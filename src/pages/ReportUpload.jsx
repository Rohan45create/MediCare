            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                <div className="max-w-3xl mx-auto">
                    <div className="mb-8">
                        ...
                        <p className="text-slate-500 dark:text-slate-400">{t('upload.subtitle')}</p>
                    </div>
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 p-8 text-center">
                        ...
                        {!file ? (
                            <div className="border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-xl p-12 transition-colors hover:bg-slate-50 dark:hover:bg-slate-800/50 relative cursor-pointer">
                                ...
                                <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-1">{t('upload.dragDrop')}</h3>
                                <p className="text-slate-500 dark:text-slate-400 text-sm">{t('upload.or')} {t('upload.browse')}</p>
                            </div>
                        ) : (
                            <div className="bg-slate-50 dark:bg-slate-800/50 p-6 rounded-xl border border-slate-200 dark:border-slate-700">
                                ...
                                {isUploading && (
                                    <div className="mt-6">
                                        <div className="flex justify-between text-sm mb-2">
                                            <span className="text-slate-600 dark:text-slate-300 font-medium">{t('upload.processing')}</span>
                                ...
                                {uploadComplete && (
                                    <div className="mt-6 p-4 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-xl flex items-start text-left">
                                        ...
                                        <div>
                                            <h4 className="font-semibold text-emerald-800 dark:text-emerald-300">{t('upload.success')}</h4>
                                ...
                                {!uploadComplete && (
                                    ...
                                    >
                                        {isUploading ? t('status.loading', { ns: 'common' }) : t('buttons.analyze', { ns: 'common' })}
                                    </button>
                                )}
                            </div>
                        )}
